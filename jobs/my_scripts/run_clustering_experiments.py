#!/usr/bin/env python3
"""
Local experiment runner for clustering experiments.
This script parses the YAML configuration and runs clustering experiments locally.
"""
...
def run_clustering_experiment(model, task, config, extra_flags, wandb_run=None):
    """Run a clustering experiment."""
    model_name = model.split('/')[-1]
    experiment_name = f"{model_name}_{task}_clustering"
...
def main():
    parser = argparse.ArgumentParser(description="Run clustering experiments locally")
    ...
    parser.add_argument("--extra-flags", default="--n_runs 1 --eval_type greedy --output_dir my_results/clustering_results --experiment_name clustering", help="Extra flags to pass to evaluation scripts")
    ...
    os.makedirs("my_results/clustering_results", exist_ok=True)
    ...
    for task in tasks:
        for model in models:
            ...
            if task in config and model in config[task]:
                try:
                    if not args.dry_run:
                        run_clustering_experiment(model, task, config[task][model], args.extra_flags, wandb_run=wandb_run)
                except subprocess.CalledProcessError as e:
                    error_msg = f"Error running clustering experiment for {model} on {task}: {e}"
                    print(error_msg)
                    if wandb_run:
