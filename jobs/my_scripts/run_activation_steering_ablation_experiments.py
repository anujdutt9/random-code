#!/usr/bin/env python3
"""
Local experiment runner for activation steering ablation studies.
This script parses the YAML configuration and runs activation steering ablation experiments locally.
"""
...
def run_activation_steering_ablation_experiment(model, task, config, extra_flags, ablate_param, ablate_value, wandb_run=None):
    """Run an activation steering ablation experiment with a specific parameter value."""
    model_name = model.split('/')[-1]
    experiment_name = f"{model_name}_{task}_activation_steering_ablation_{ablate_param}_{ablate_value}"
    ...
    if wandb_run:
        wandb_run.log({
            "experiment_type": "activation_steering_ablation",
            "model": model,
            "task": task,
...
def main():
    parser = argparse.ArgumentParser(description="Run activation steering ablation experiments locally")
    ...
    parser.add_argument("--extra-flags", default="--n_runs 1 --eval_type greedy --output_dir my_results/activation_steering_ablation_results --experiment_name activation_steering_ablation", help="Extra flags to pass to evaluation scripts")
    parser.add_argument("--dry-run", action="store_true", help="Print commands without running them")
    
    # Ablation specific arguments
    parser.add_argument("--ablate-param", required=True, 
                       choices=["n_contrastive_samples", "num_fewshot_examples", "multipliers", "layers"],
    ...
    print(f"Running activation steering ablation experiments")
    print(f"Tasks: {tasks}")
    ...
    os.makedirs("my_results/activation_steering_ablation_results", exist_ok=True)
    
    for task in tasks:
        for model in models:
            print(f"\nProcessing {model} on {task}")
            
            # Run activation steering ablation experiments for each value
            if task in config and model in config[task]:
                for ablate_value in args.ablate_values:
                    print(f"\n  Running activation steering ablation with {args.ablate_param} = {ablate_value}")
                    try:
                        if not args.dry_run:
                            run_activation_steering_ablation_experiment(
                                model, task, config[task][model], 
                                args.extra_flags, args.ablate_param, 
                                ablate_value, wandb_run=wandb_run
                            )
                    except subprocess.CalledProcessError as e:
                        error_msg = f"Error running activation steering ablation experiment for {model} on {task} with {args.ablate_param}={ablate_value}: {e}"
                        print(error_msg)
                        if wandb_run:
    ...
    print("\n" + "=" * 50)
    print("All activation steering ablation experiments completed!")
